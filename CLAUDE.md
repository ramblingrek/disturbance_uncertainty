...
- `_extract_windows()` — shared backbone; places non-overlapping W×W windows per stack using random sequential adsorption, accepted only if no overlap with prior accepted centers (`|Δrow|<W` AND `|Δcol|<W`). Interpreter field thresholded at 50% (with optional `InterpreterAgent` perturbation) before patch extraction. `window_size=1` degenerates to simple random per-pixel sampling. Two placement modes:
  - **Unstratified** (default): `n_windows` candidate centers drawn uniformly at random from the valid inset region. `max_attempts` (default `n_windows×50`) caps the search; warns and returns partial results if unreachable.
  - **Reference-stratified**: pass `n_windows_per_stratum` (dict, e.g. `{0: 50, 1: 50}`) to stratify candidate centers by the *true* interpreter label at the center pixel instead — not the map. Candidate pools per stratum are precomputed and shuffled, then placed interleaved round-robin (so one stratum doesn't spatially exhaust the raster before the other's quota is filled), sharing one non-overlap check across strata. `n_windows`/`max_attempts` are ignored in this mode; warns per-stratum if a quota can't be reached (e.g. a validation stack with zero truly-disturbed pixels). Returned window dicts include a `"stratum"` key (the interp label the center was drawn for, `None` in unstratified mode). **Do not feed reference-stratified samples into `olofsson_area_estimates`** — its area formula assumes the sample is drawn map-stratified (or unstratified/SRS, which it also handles correctly); under reference-stratification the marginal area estimate that formula targets becomes an exact algebraic identity to the known reference-stratum population size, not an actual estimate, so it's degenerate rather than merely biased. Use the raw/unweighted confusion matrix and `pixel_level_metrics` on the sample instead (see `interpreter/notebooks/01_map_comparison_benchmark.ipynb` step 4).
- `window_sample_A()` — every pixel in every window → one `(map_class, ref_class)` row; same schema as `binary_stratified_sample`, feeds into `binary_confusion_from_samples` / `olofsson_area_estimates` (map-stratified or unstratified samples only — see caveat above)
